...
import { Dumbbell, Calendar, Target, X, Star, Video, Zap, Moon } from "lucide-react";
...
const WorkoutPlanModal = ({ plan, onClose }: { plan: WorkoutPlan | null; onClose: () => void; }) => {
  ...
  return (
    <Dialog open={!!plan} onOpenChange={(open) => !open && onClose()} modal={true}>
...
const WorkoutsPage = () => {
  ...
  return (
    <Layout>
            ...
            {filteredPlans.length === 0 ? (
              ...
              >
                <div className="p-8 bg-muted/30 rounded-2xl border border-dashed border-border/50">
                  ...
                  <p className="text-muted-foreground text-2xl font-semibold">No workout plans found matching your criteria.</p>
                  <p className="text-muted-foreground mt-2">Try adjusting your filters to discover more options.</p>
                </div>
              </motion.div>
            ) : (
      ...
      <WorkoutPlanModal plan={activePlan} onClose={() => setActivePlan(null)} />
    </Layout>
  );
};
